...
select sum(total_exec_time)/sum(calls) as mean_query_time
     , sum(calls) as total_calls
     , sum(calls)::float/(select sum(calls) from filtered_stat)*100 as call_pct
     , sum(total_exec_time) as cumulative_time
     , sum(total_exec_time)::float/(select sum(total_exec_time) from filtered_stat)*100 as cumulative_time_pct
